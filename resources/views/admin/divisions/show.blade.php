<x-app-layout>
    ...
    {{-- TAMBAHKAN deleteDivisionModalOpen ke x-data --}}
    <div class="py-12" x-data="{ removeModalOpen: false, deleteDivisionModalOpen: false, selectedMemberName: '', removeUrl: '' }">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="space-y-6">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700/50 px-6 py-4 border-t border-gray-100 dark:border-gray-700">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-gray-900 dark:text-white truncate">{{ $division->head->name }}</p>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 truncate">{{ $division->head->email }}</p>
                                </div>
                            </div>
                            @else
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <a href="{{ route('admin.divisions.edit', $division) }}" class="inline-flex justify-center gap-1 items-center px-4 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md font-semibold text-xs text-gray-700 dark:text-gray-300 uppercase tracking-widest shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150">
                        ...
                        {{-- MODIFIKASI: Tombol Hapus Divisi memicu Modal --}}
                        <button type="button"
                            {{-- Kita menggunakan $dispatch untuk berkomunikasi dengan x-data di wrapper utama, atau akses langsung scope variabel --}}
                            @click="deleteDivisionModalOpen = true"
                    ...
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-xl border border-gray-200 dark:border-gray-700">
                        <div class="p-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300">{{ $division->members->count() }}</span>
                            </div>
                        </div>
                        ...
                        <div class="p-6 bg-gray-50 dark:bg-gray-700/20 border-b border-gray-100 dark:border-gray-700">
                            <form action="{{ route('admin.divisions.addMember', $division) }}" method="POST">
                                ...
                                <div class="flex flex-col sm:flex-row gap-3">
                                    ...
                                    <x-primary-button class="justify-center py-2.5">{{ __('Tambah') }}</x-primary-button>
                                </div>
                            </form>
                        </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-800">
                                    <tr>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Nama Karyawan</th>
                                ...
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 h-9 w-9">
                                                    <div class="h-9 w-9 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-gray-500 dark:text-gray-300 font-bold text-xs border-2 border-white dark:border-gray-800">{{ substr($member->name, 0, 1) }}</div>
                                                </div>
                                                <div class="ml-4">
                                                    <div class="text-sm font-medium text-gray-900 dark:text-gray-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{{ $member->name }}</div>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800 dark:bg-indigo-900/50 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">Ketua Tim</span>
                                            @else
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300">Anggota</span>
                                            @endif
                                        </td>
                                    ...
                                    @empty
                                    <tr>
                                        <td colspan="3" class="px-6 py-10 text-center text-gray-500 dark:text-gray-400">Belum ada anggota</td>
                                    </tr>
                                    @endforelse
                                </tbody>
                            </table>
        ...
        {{-- MODAL 1: KONFIRMASI HAPUS ANGGOTA (Yang sudah ada) --}}
                ...
                <div class="relative transform overflow-hidden rounded-lg bg-white dark:bg-gray-800 text-left shadow-xl transition-all sm:max-w-lg w-full">
                    <div class="bg-white dark:bg-gray-800 px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                <h3 class="text-lg font-semibold leading-6 text-gray-900 dark:text-white">Keluarkan Anggota</h3>
                                <p class="text-sm text-gray-500 mt-2">Apakah Anda yakin ingin mengeluarkan <span class="font-bold text-gray-900 dark:text-white" x-text="selectedMemberName"></span>?</p>
                            </div>
                        </div>
                    </div>
                    <div class="bg-gray-50 dark:bg-gray-700/30 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-3">
                        <form :action="removeUrl" method="POST">
                            @csrf
                            <button type="submit" class="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:w-auto">Ya, Keluarkan</button>
                        </form>
                        <button type="button" @click="removeModalOpen = false" class="mt-3 inline-flex w-full justify-center rounded-md bg-white dark:bg-gray-700 px-3 py-2 text-sm font-semibold text-gray-900 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600 sm:mt-0 sm:w-auto">Batal</button>
                    </div>
                </div>
            </div>
        </div>

        {{-- MODAL 2: KONFIRMASI HAPUS DIVISI (BARU) --}}
        <div x-show="deleteDivisionModalOpen" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" x-transition.opacity>
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 dark:bg-gray-900 dark:bg-opacity-75" @click="deleteDivisionModalOpen = false"></div>
            <div class="flex min-h-full items-center justify-center p-4">
                <div class="relative transform overflow-hidden rounded-lg bg-white dark:bg-gray-800 text-left shadow-xl transition-all sm:max-w-lg w-full">
                    <div class="bg-white dark:bg-gray-800 px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                <h3 class="text-lg font-semibold leading-6 text-gray-900 dark:text-white">Hapus Divisi?</h3>
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500 dark:text-gray-400">
                                        Apakah Anda yakin ingin menghapus divisi <strong>{{ $division->name }}</strong>?
                                    </p>
                                    <p class="text-sm text-red-500 dark:text-red-400 font-medium mt-2">
                                        Perhatian: Seluruh anggota akan kehilangan status divisinya (menjadi non-divisi). Tindakan ini tidak dapat dikembalikan.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700/30 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-3">
                        ...
                        <button type="button" @click="deleteDivisionModalOpen = false" class="mt-3 inline-flex w-full justify-center rounded-md bg-white dark:bg-gray-700 px-3 py-2 text-sm font-semibold text-gray-900 dark:text-gray-200 shadow-sm ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600 sm:mt-0 sm:w-auto transition">
                            Batal
                        </button>
                    </div>
                </div>
